    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        #login-screen {
            ...
            background: var(--bg-light); z-index: 5000; display: flex; align-items: center; justify-content: center;
        }
...
<body onload="checkLoginState(); checkNotifications(); loadLinkSettings(); loadUpdateSettings(); renderStaff();">

<div id="login-screen">
    <div class="login-card">
        <h2 style="color:var(--text-dark)">تسجيل الدخول <i class="fas fa-lock"></i></h2>
        <input type="text" id="login-user" placeholder="اسم المستخدم">
        <input type="password" id="login-pass" placeholder="كلمة المرور">
        <button class="btn-action bg-save" onclick="handleLogin()" style="width:100%; margin-top:15px">دخول</button>
        <p style="font-size:0.8rem; color:#888; margin-top:15px">تنبيه: الحساب الافتراضي هو admin وكلمة المرور 123</p>
    </div>
</div>
...
<div id="sidebar" class="sidebar">
    <div class="sidebar-header">
        ...
        <p id="sidebar-user-login"><i class="fas fa-user-circle"></i> حساب الموظف</p>
    </div>
    <div class="sidebar-menu">
        ...
        <a href="#" onclick="logout();" style="margin-top:20px; color: #e74c3c;"><i class="fas fa-sign-out-alt"></i> خروج</a>
    </div>
</div>
...
<div class="container">
    ...
    <div id="cloud-screen" class="screen">
        <button class="btn-action bg-back" onclick="showSection('home-screen')">رجوع</button>
        <div class="inner-card">
            <h3 style="color:#2c3e50"><i class="fas fa-cloud-download-alt"></i> تحديث التطبيق</h3>
            <p>يمكنك حفظ رابط النسخة الجديدة هنا للوصول السريع:</p>
            <input type="text" id="update-url" placeholder="ضع رابط التحديث (مثلاً: رابط ميديا فاير أو جوجل درايف)">
            <button class="btn-action bg-save" onclick="saveUpdateUrl()">حفظ الرابط</button>
            <hr>
            <button class="btn-action bg-print" onclick="checkForUpdate()"><i class="fas fa-sync"></i> الانتقال إلى رابط التحديث</button>
        </div>
    </div>
    ...
    <div id="accounts-screen" class="screen">
        ...
        <div class="inner-card">
            ...
            <div class="summary-box" style="background:var(--g-sales)">
                <div class="summary-title">إجمالي مبيعات اليوم</div>
                <div class="summary-val" id="day-sales-total">0 د.ع</div>
            </div>
        </div>
        <div class="inner-card">
            <h3 style="color:#2c3e50"><i class="fas fa-calendar-alt"></i> الحسابات الشهرية</h3>
            <div class="summary-box" style="background:var(--g-purchases); margin-bottom:15px;">
                <div class="summary-title">إجمالي مشتريات الشهر</div>
                <div class="summary-val" id="month-purchases-total">0 د.ع</div>
            </div>
            <div class="summary-box" style="background:var(--g-reports)">
                <div class="summary-title">إجمالي مبيعات الشهر</div>
                <div class="summary-val" id="month-sales-total">0 د.ع</div>
            </div>
        </div>
    </div>
    ...
    <div id="store-screen" class="screen">
        ...
        <div class="inner-card">
            ...
            <div class="btn-row">
                ...
                <button class="btn-action" style="background:#0088cc" onclick="sendTelegramReport()">إرسال تقرير تجريبي</button>
            </div>
        </div>
    </div>
</div>
...
<footer><p>© 2026 كاشير برو - النسخة v1.5</p></footer>
...
<script>
    ...
    function handleLogin() {
        ...
        if (user === 'admin' && pass === '123') {
            currentUser = { name: 'المدير العام', role: 'كامل الصلاحيات' };
            sessionStorage.setItem('pos_current_user', JSON.stringify(currentUser));
            checkLoginState();
            return;
        }

        // البحث في حسابات الموظفين
    ...
    function applyPermissions() {
        document.getElementById('sidebar-user-login').innerHTML = `<i class="fas fa-user-circle"></i> ${currentUser.name} (${currentUser.role})`;
        
        // إخفاء العناصر بناءً على الصلاحية
        const role = currentUser.role;
        
        // المدير يرى كل شيء
        if (role === 'كامل الصلاحيات') return;

        // صلاحيات الكاشير
        if (role === 'مبيعات فقط') {
            document.getElementById('card-purchases').style.display = 'none';
            document.getElementById('card-inventory').style.display = 'none';
            document.getElementById('card-store').style.display = 'none';
            document.getElementById('card-accounts').style.display = 'none';
            document.getElementById('side-staff-link').style.display = 'none';
            document.getElementById('side-store-link').style.display = 'none';
        }

        // صلاحيات أمين المخزن
        if (role === 'مخزن فقط') {
            document.getElementById('card-sales').style.display = 'none';
    ...
    // --- نظام القائمة الجانبية والموظفين ---
    ...
    function addNewStaff() {
        ...
        if(!name || !user || !pass) return alert("يرجى ملء كافة بيانات الموظف");
    ...
    function renderStaff() {
        ...
        if(!container) return;
        container.innerHTML = staffMembers.length === 0 ? "<p style='text-align:center; color:#999;'>لا يوجد موظفين مضافين</p>" : "";
        staffMembers.forEach((s, idx) => {
            container.innerHTML += `
                <div class="staff-item">
                    <div><strong>${s.name}</strong><br><small>المستخدم: ${s.user} | الصلاحية: ${s.role}</small></div>
                    <button class="edit-btn" style="background:#e74c3c" onclick="deleteStaff(${idx})"><i class="fas fa-trash"></i></button>
                </div>`;
        });
    }
    ...
    function saveUpdateUrl() {
        ...
        localStorage.setItem('pos_update_url', url);
        alert("تم حفظ رابط التحديث بنجاح");
    }
    function loadUpdateSettings() {
        const url = localStorage.getItem('pos_update_url') || "";
        document.getElementById('update-url').value = url;
    }
    function checkForUpdate() {
        const url = localStorage.getItem('pos_update_url');
        if(url && url !== "") { window.open(url, '_blank'); } 
        else { alert("يرجى وضع رابط التحديث أولاً"); }
    }
    ...
    function checkNotifications() {
        ...
        inventory.forEach(item => {
            if (item.qty <= 5) {
                notifications.push({ type: 'warning', icon: 'fa-exclamation-triangle', text: `المادة <strong>${item.name}</strong> شارفت على النفاد (المتبقي: ${item.qty})` });
            }
        });
        let itemSalesCount = {};
        salesInvoices.forEach(inv => { inv.items.forEach(it => { itemSalesCount[it.name] = (itemSalesCount[it.name] || 0) + it.qty; }); });
        for (let name in itemSalesCount) { if (itemSalesCount[name] >= 10) { notifications.push({ type: 'info', icon: 'fa-bolt', text: `مادة سريعة البيع: <strong>${name}</strong> (تم بيع ${itemSalesCount[name]} قطع)` }); } }
        if (notifications.length > 0) {
            dot.style.display = 'block';
            container.innerHTML = notifications.map(n => `<div class="notif-item notif-${n.type}"><i class="fas ${n.icon} fa-lg"></i><div>${n.text}</div></div>`).join('');
        } else {
    ...
    function saveLinkSettings() {
        const settings = { tgToken: document.getElementById('tg-token').value, tgChatId: document.getElementById('tg-chatid').value, waLink: document.getElementById('wa-channel-link').value };
        localStorage.setItem('pos_links', JSON.stringify(settings));
        alert("تم الحفظ");
    }
    function loadLinkSettings() {
        const settings = JSON.parse(localStorage.getItem('pos_links')) || {};
        document.getElementById('tg-token').value = settings.tgToken || '';
        document.getElementById('tg-chatid').value = settings.tgChatId || '';
        document.getElementById('wa-channel-link').value = settings.waLink || '';
    }
    async function sendTelegramReport() {
        const settings = JSON.parse(localStorage.getItem('pos_links'));
        if(!settings || !settings.tgToken) return alert("أكمل الإعدادات");
        calculateAccounts();
        const text = `📊 تقرير\nمبيعات اليوم: ${document.getElementById('day-sales-total').innerText}`;
        const url = `https://api.telegram.org/bot${settings.tgToken}/sendMessage?chat_id=${settings.tgChatId}&text=${encodeURIComponent(text)}`;
        await fetch(url); alert("تم الإرسال");
    }
    function showSection(id) { stopCamera(); document.querySelectorAll('.screen').forEach(s => s.classList.remove('active')); document.getElementById(id).classList.add('active'); if(id === 'inventory-screen') renderInventory(); if(id === 'accounts-screen') calculateAccounts(); if(id === 'notif-screen') checkNotifications(); if(id === 'staff-management-screen') renderStaff(); window.scrollTo(0,0); }
    function calculateAccounts() {
        const todayStr = new Date().toLocaleDateString('ar-EG');
        const currentMonth = new Date().getMonth();
        const currentYear = new Date().getFullYear();
        let daySales = salesInvoices.filter(inv => inv.date.includes(todayStr)).reduce((sum, inv) => sum + inv.total, 0);
        let monthSales = salesInvoices.filter(inv => { let parts = inv.date.split(',')[0].split('/'); let invDate = new Date(parts[2], parts[1] - 1, parts[0]); return invDate.getMonth() === currentMonth && invDate.getFullYear() === currentYear; }).reduce((sum, inv) => sum + inv.total, 0);
        let monthPurchases = purchaseInvoices.filter(inv => { let parts = inv.date.split(',')[0].split('/'); let invDate = new Date(parts[2], parts[1] - 1, parts[0]); return invDate.getMonth() === currentMonth && invDate.getFullYear() === currentYear; }).reduce((sum, inv) => sum + inv.total, 0);
        document.getElementById('day-sales-total').innerText = daySales.toLocaleString() + " د.ع";
        document.getElementById('month-sales-total').innerText = monthSales.toLocaleString() + " د.ع";
        document.getElementById('month-purchases-total').innerText = monthPurchases.toLocaleString() + " د.ع";
    }
    ...
    function savePurchase() {
        if(tempPurchases.length === 0) return alert("فارغة");
    ...
    function searchProductByBarcode(code) { const product = inventory.find(i => i.barcode === code); if (product) { document.getElementById('s-item-name').value = product.name; document.getElementById('s-price').value = product.price; } }
    ...
    function saveSale() {
        if(tempSales.length === 0) return alert("فارغة");
        const invoice = { id: "SAL-" + Date.now(), customer: document.getElementById('s-customer').value || "زبون نقدي", date: new Date().toLocaleString('ar-EG'), items: [...tempSales], total: tempSales.reduce((sum, i) => sum + (i.qty * i.price), 0), type: 'مبيعات' };
        salesInvoices.push(invoice);
        localStorage.setItem('pos_s_invoices', JSON.stringify(salesInvoices));
        tempSales.forEach(saleItem => { let idx = inventory.findIndex(i => i.name === saleItem.name); if(idx > -1) inventory[idx].qty -= saleItem.qty; });
        localStorage.setItem('pos_inventory', JSON.stringify(inventory));
        tempSales = []; alert("تم البيع"); showSection('home-screen');
    }
    function renderInventory() {
        const container = document.getElementById('inventory-container');
        container.innerHTML = inventory.length === 0 ? "<p style='text-align:center;'>المخزن فارغ</p>" : "";
        inventory.forEach((item, index) => {
            container.innerHTML += `<div class="inventory-item"><div><strong>${item.name}</strong><br><small>العدد: ${item.qty} | السعر: ${Number(item.price).toLocaleString()} د.ع</small></div><button class="edit-btn" onclick="openEditModal(${index})">تعديل</button></div>`;
        });
    }
    function openEditModal(index) { currentEditIdx = index; const item = inventory[index]; document.getElementById('edit-item-name').innerText = item.name; document.getElementById('edit-item-qty').value = item.qty; document.getElementById('edit-item-price').value = item.price; document.getElementById('edit-modal').style.display = 'flex'; }
    function saveInventoryEdit() { if(currentEditIdx === null) return; inventory[currentEditIdx].qty = parseFloat(document.getElementById('edit-item-qty').value) || 0; inventory[currentEditIdx].price = parseFloat(document.getElementById('edit-item-price').value) || 0; localStorage.setItem('pos_inventory', JSON.stringify(inventory)); renderInventory(); closeEditModal(); alert("تم التعديل"); }
    function closeEditModal() { document.getElementById('edit-modal').style.display = 'none'; currentEditIdx = null; }
    function renderReports(type) {
        const container = document.getElementById('reports-list-container');
        const data = (type === 'purchases') ? purchaseInvoices : salesInvoices;
        container.innerHTML = data.length === 0 ? "<p style='text-align:center;'>لا توجد بيانات</p>" : "";
        [...data].reverse().forEach(inv => {
            container.innerHTML += `<div class="report-box" onclick="viewInvoiceDetails('${type}', '${inv.id}')"><div class="report-header"><strong>${inv.customer}</strong><span>${inv.total.toLocaleString()} د.ع</span></div><div>${inv.id} | ${inv.date}</div></div>`;
        });
    }
    function viewInvoiceDetails(type, id) {
        const data = (type === 'purchases') ? purchaseInvoices : salesInvoices;
        const inv = data.find(i => i.id === id); if(!inv) return;
        document.getElementById('modal-title').innerText = "فاتورة " + inv.type;
        document.getElementById('modal-info').innerHTML = `<p>الجهة: ${inv.customer}</p><p>التاريخ: ${inv.date}</p>`;
        let html = "<hr>"; inv.items.forEach(item => html += `<div style="display:flex; justify-content:space-between;"><span>${item.name} (${item.qty})</span><span>${(item.qty * item.price).toLocaleString()} د.ع</span></div>`);
